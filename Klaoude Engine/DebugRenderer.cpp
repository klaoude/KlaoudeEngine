...
namespace KlaoudeEngine
{
	...
	void DebugRenderer::end()
	{
		glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
		//Orphan
		glBufferData(GL_ARRAY_BUFFER, m_verts.size() * sizeof(DebugVertex), nullptr,
			GL_DYNAMIC_DRAW);
		//Upload
		glBufferSubData(GL_ARRAY_BUFFER, 0, m_verts.size() * sizeof(DebugVertex),
			m_verts.data());
		//Unbind
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_ibo);
		//Orphan
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indices.size() * sizeof(GLuint), nullptr,
			GL_DYNAMIC_DRAW);
		//Upload
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, m_indices.size() * sizeof(GLuint),
			m_indices.data());
		//Unbind
